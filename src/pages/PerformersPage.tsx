...
export function PerformersPage() {
  ...
  const locations = useMemo(() => {
    ...
    festivalData.performers.forEach(p => locs.add(p.location));
    return Array.from(locs);
  }, []);

  const availableDays = useMemo(() => {
    const days = new Set<Day>();
    festivalData.performers.forEach(event => {
  ...
  const filteredEvents = useMemo(() => {
    return festivalData.performers
      .filter(event => {
      ...
      .map(event => ({ ...event, day: event.day as Day }))
      .sort((a, b) => {
          ...
          <div key={day} className="day-group">
            ...
            {events.map((event, idx) => (
              <EventCard
                ...
                id={generateEventId('performers', event.day, event.name, event.startTime, event.location)}
                name={event.name}
